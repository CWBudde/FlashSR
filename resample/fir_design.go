...
// ErrInvalidRatio is returned when the up or down factor is ≤ 0.
var ErrInvalidRatio = errors.New("resample: invalid ratio")

// designPolyphaseFIR designs a Kaiser-windowed polyphase FIR prototype and
// splits it into per-phase sub-filters.
func designPolyphaseFIR(up, down int, cfg config) (taps []float64, phases [][]float64, maxPhaseLn int, err error) {
	if up <= 0 || down <= 0 {
...
// approximateRatio converts a floating-point rate ratio to a reduced integer
// fraction num/den using the Stern-Brocot / continued-fraction method.
...
// kaiserWindow computes the Kaiser window coefficient at index i of an n-point window.
